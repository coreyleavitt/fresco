...
proc settled*(r: HeadlessResult): bool =
  ...
  ## the one-expression "clean run" assertion.
...
Drain-mode event loop, per event:
- **App-death short-circuit:** if `appFut.finished` before injection, stop injecting (a dead app must not be raced through the remaining script — under drain every framework clause goes vacuously idle and the harness would sprint through a dead run in silence); if it failed, record `appError`.
- Inject, then `drainToIdle(screen, settle.drain)`; on `DrainTimeoutError`, record a `SettleFailure` and short-circuit remaining events.
- After the app-finish/cancel dance, one final **best-effort** drain (its timeout records `site = sfFinalDrain`, never raises out), then `teardownFlush()` + `paint()` + capture, unconditionally — exactly as today.
