...
- **L4 guard (Low, accumulated-Lows report, fixed 2026-08-07):** `pendingCallbacksCount`'s zero-reading exactness depends on the `not chronosStrictReentrancy` build (it subtracts the dispatcher's permanent `SentinelCallback`; under `chronosStrictReentrancy` the sentinel is counted too, so the accessor never reads zero and every `drainToIdle` spins to its deadline) — `chronosStrictReentrancy` defaults to `defined(chronosPreviewV5)`, an unguarded build-flag dependency this RFC hadn't stated. `headless/runner.nim` now imports `chronosStrictReentrancy` from `chronos/config` directly and fails the build with `{.error.}` if it is ever `true`, naming `dcDispatcher`'s exact-witness assumption in the message, rather than letting the drain silently degrade to always-timeout.
