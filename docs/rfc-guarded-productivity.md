...
> **The claim:** the explicit-deps graph plus a delay/guard marker is enough to decide, at compile time, whether a cyclic reactive structure is productive — turning "no cycles allowed" into "exactly the productive cycles allowed."

## Nim leverage

- **Typed macros** to detect cycles in the explicit-deps graph and check each cycle for an intervening guard.
...
Paper on guarded productivity for reactive derivations — adapting guarded recursion / coinductive productivity to a signals substrate, decided from the static dependency graph. Novelty: the substrate setting and the compile-time decision from the C-shape walker, not guarded recursion itself.
