...
suite "bindRow":
  ...
  test "scope dispose stops the binding":
    ...
    check r.rows[0] == "alpha"
    dispose(root)
    label.set("beta")
    check r.rows[0] == "alpha"   # frozen at last value
  ...
  test "multiple bindRow on same region run independently":
    ...
    discard createRoot:
      bindRow r, 0, [a]: a
      bindRow r, 1, [b]: b
    check r.rows[0] == "A" and r.rows[1] == "B"
    a.set("AA")
    check r.rows == @["AA", "B"]
    b.set("BB")
    check r.rows == @["AA", "BB"]
...
suite "bindCollection: wmFromEnd (tail-window, #41)":
  ...
  test "push when filled shifts window forward by one":
    ...
    check r.rows == @["1", "2", "3"]
    c.push(4)
    check r.rows == @["2", "3", "4"]   # 1 pushed off the top
    c.push(5)
    check r.rows == @["3", "4", "5"]
  ...
  test "push before fill: appears at next row, no shift":
    ...
    discard createRoot:
      bindCollection(r, 0..<5, c, proc(x: int): string = $x, mode = wmFromEnd)
    c.push(3)
    check r.rows == @["1", "2", "3", "", ""]   # still top-down
    c.push(4)
    check r.rows == @["1", "2", "3", "4", ""]
    c.push(5)
    check r.rows == @["1", "2", "3", "4", "5"]
    # Now filled; next push shifts.
    c.push(6)
    check r.rows == @["2", "3", "4", "5", "6"]
  ...
  test "wmFromStart (default) regression: behavior unchanged":
    ...
    check r.rows == @["1", "2", "3"]    # from-start: first 3
