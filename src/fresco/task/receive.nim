...
macro receive*(stream: untyped, body: untyped): untyped =
  ...
  # **CLS save/restore around every await this macro emits.** The
  # `{.task.}` pragma rewrites awaits in the enclosing proc's body
  # before macro expansion, so the awaits below — generated by this
  # macro at expansion time — are invisible to task's rewriter and
  # need explicit context preservation.
  let ctxSym = genSym(nskLet, "frescoCtx")

  if afterDur == nil:
    # Simple form: `let ev = await nextKey()`. The `try/finally` wraps
    # only the await so the captured context is restored on every
    # exit path (normal completion, failure, cancellation).
    result = quote do:
    ...
    # fires first.
    #
    # The outer try/finally guarantees both futures are cancelled on
    # every exit path — including a `CancelledError` propagating out
    # of the race await itself, which would otherwise orphan the
    # `nextKey` future and silently consume the next keypress with
    # no consumer.
    #
